...
uint32_t BTFStringTable::addString(StringRef S) {
  ...
  StringToOffset[Table.back()] = Offset;
  Size += S.size() + 1;
  return Offset;
}
...
BTFDebug::BTFDebug(AsmPrinter *AP)
    : DebugHandlerBase(AP), OS(*Asm->OutStreamer), SkipInstruction(false),
      ...
      CompletedTypeEntries(0) {
  addString("\0");
}

bool BTFDebug::debugEnabled() const { return std::getenv("LLD_BTF_DEBUG"); }

void BTFDebug::logDebugStats(StringRef Phase) const {
  if (!debugEnabled())
    return;

  uint64_t FuncInfos = 0;
  for (const auto &Entry : FuncInfoTable)
    FuncInfos += Entry.second.size();

  uint64_t LineInfos = 0;
  for (const auto &Entry : LineInfoTable)
    LineInfos += Entry.second.size();

  uint64_t FieldRelocs = 0;
  for (const auto &Entry : FieldRelocTable)
    FieldRelocs += Entry.second.size();

  errs() << "btf-codegen-debug: " << Phase
         << " module=" << MMI->getModule()->getModuleIdentifier()
         << " types=" << TypeEntries.size()
         << " strings=" << StringTable.stringsCount()
         << " string-bytes=" << StringTable.getSize()
         << " data-secs=" << DataSecEntries.size()
         << " structs=" << StructTypes.size()
         << " proto-funcs=" << ProtoFunctions.size()
         << " compile-units=" << ProcessedCUs.size()
         << " fixup-composites=" << FixupDerivedTypes.size()
         << " func-infos=" << FuncInfos << " line-infos=" << LineInfos
         << " field-relocs=" << FieldRelocs << "\n";
}
...
void BTFDebug::beginFunctionImpl(const MachineFunction *MF) {
  ...
  processCompileUnitTypes(Unit);

  // Collect local types in this function from retained nodes, IR debug
  // intrinsics/records, and live debug value history. Retained nodes keep
  // optimized-away arguments visible, while the IR and machine-level streams
  // cover concrete function-local records that never make it into
  // retainedNodes.
  std::unordered_map<uint32_t, StringRef> FuncArgNames;
  auto VisitLocalVar = [&](const DILocalVariable *DV) {
    if (!DV)
      return;
    ...
    uint32_t Arg = DV->getArg();
    if (!Arg || FuncArgNames.find(Arg) != FuncArgNames.end())
      return;

    const auto *VarSP = getDISubprogram(DV->getScope());
    if (VarSP && VarSP->describes(&MF->getFunction()))
  ...
  for (const BasicBlock &BB : MF->getFunction()) {
    for (const Instruction &I : BB) {
      if (const auto *DVI = dyn_cast<DbgVariableIntrinsic>(&I))
        VisitLocalVar(DVI->getVariable());

      for (const DbgRecord &DR : I.getDbgRecordRange())
        if (const auto *DVR = dyn_cast<DbgVariableRecord>(&DR))
          VisitLocalVar(DVR->getVariable());
    }
  }

  for (const auto &VarEntries : DbgValues)
    if (const auto *DV = dyn_cast<DILocalVariable>(VarEntries.first.first))
      VisitLocalVar(DV);
...
void BTFDebug::processCompileUnitTypes(const DICompileUnit *Unit) {
  if (!Unit || !ProcessedCUs.insert(Unit).second)
    return;

  for (const DICompositeType *EnumTy : Unit->getEnumTypes())
    visitTypeEntry(EnumTy);

  for (const DIScope *Scope : Unit->getRetainedTypes()) {
    if (const auto *Ty = dyn_cast<DIType>(Scope))
      visitTypeEntry(Ty);
  }
}

void BTFDebug::completeNewTypes() {
  while (CompletedTypeEntries < TypeEntries.size())
    TypeEntries[CompletedTypeEntries++]->completeType(*this);
}

void BTFDebug::endModule() {
  auto Start = std::chrono::steady_clock::now();
  auto LogPhase = [&](StringRef Phase) {
    if (!debugEnabled())
      return;
  ...
  LogPhase("end-module-start");
  for (DICompileUnit *CUNode : MMI->getModule()->debug_compile_units())
    processCompileUnitTypes(CUNode);
  LogPhase("post-cu-types");

  processGlobals();
  LogPhase("post-globals");

  for (auto &DataSec : DataSecEntries)
    addType(std::move(DataSec.second));
  LogPhase("post-datasecs");

  StringMap<uint32_t> StructTypeIds;
  for (const auto &StructType : StructTypes)
    StructTypeIds.try_emplace(StructType->getName(), StructType->getId());
  ...
  for (auto &Fixup : FixupDerivedTypes) {
    ...
    auto StructIt = StructTypeIds.find(TypeName);
    if (StructIt != StructTypeIds.end())
      StructTypeId = StructIt->second;
  ...
  LogPhase("post-fixups");

  // Complete BTF type cross references.
  completeNewTypes();
  LogPhase("post-complete");

  // Emit BTF sections.
  emitBTFSection();
  LogPhase("post-btf-section");
  emitBTFExtSection();
  LogPhase("post-btf-ext-section");
}
