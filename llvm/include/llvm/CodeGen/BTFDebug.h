...
public:
  ...
  size_t stringsCount() const { return Table.size(); }
...
protected:
  ...
  std::set<const DICompileUnit *> ProcessedCUs;
  ...
  /// Collect compile-unit-level types that are not reachable through globals or
  /// function signatures alone, such as anonymous enums and retained types.
  void processCompileUnitTypes(const DICompileUnit *Unit);
  void completeNewTypes();
  ...
  void logDebugStats(StringRef Phase) const;
