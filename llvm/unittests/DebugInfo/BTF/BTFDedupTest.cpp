...
static unsigned countTypesByNameAndKind(const BTFBuilder &B, StringRef Name,
                                        uint32_t Kind) {
  unsigned Count = 0;
  for (uint32_t Id = 1; Id <= B.typesCount(); ++Id) {
    const BTF::CommonType *T = B.findType(Id);
    if (T && T->getKind() == Kind && B.findString(T->NameOff) == Name)
      ++Count;
  }
  return Count;
}

static const BTF::CommonType *findTypeByNameAndKind(const BTFBuilder &B,
                                                    StringRef Name,
                                                    uint32_t Kind) {
  for (uint32_t Id = 1; Id <= B.typesCount(); ++Id) {
    const BTF::CommonType *T = B.findType(Id);
...
TEST(BTFDedupTest, enumDeclDedupPrefersCompleteDefinition) {
  BTFBuilder B;
  uint32_t EnumS = B.addString("dax_access_mode");
  uint32_t ReadS = B.addString("DAX_ACCESS");
  uint32_t WriteS = B.addString("DAX_RECOVERY_WRITE");

  // Type 1: complete enum definition.
  B.addType({EnumS, mkInfo(BTF::BTF_KIND_ENUM) | 2, {4}});
  B.addTail(BTF::BTFEnum({ReadS, 0}));
  B.addTail(BTF::BTFEnum({WriteS, 1}));

  // Type 2: incomplete enum declaration from another object.
  uint32_t EnumDeclS = B.addString("dax_access_mode");
  B.addType({EnumDeclS, mkInfo(BTF::BTF_KIND_ENUM), {0}});

  // Type 3: pointer to the incomplete declaration.
  B.addType({0, mkInfo(BTF::BTF_KIND_PTR), {2}});

  ASSERT_SUCCEEDED(BTF::dedup(B));
  EXPECT_EQ(countTypesByNameAndKind(B, "dax_access_mode",
                                    BTF::BTF_KIND_ENUM),
            1u);

  const BTF::CommonType *Enum =
      findTypeByNameAndKind(B, "dax_access_mode", BTF::BTF_KIND_ENUM);
  ASSERT_TRUE(Enum);
  EXPECT_EQ(Enum->Size, 4u);
  EXPECT_EQ(Enum->getVlen(), 2u);

  const BTF::CommonType *Ptr = B.findType(2);
  ASSERT_TRUE(Ptr);
  EXPECT_EQ(Ptr->getKind(), BTF::BTF_KIND_PTR);
  EXPECT_EQ(Ptr->Type, 1u);
}
...
TEST(BTFDedupTest, fwdResolutionIgnoresEnumNameCollisions) {
  BTFBuilder B;
  uint32_t PhyStructS = B.addString("phy");
  uint32_t FieldS = B.addString("id");
  uint32_t IntS = B.addString("int");
  uint32_t SignedEnumS = B.addString("phy");
  uint32_t SignedEnumValS = B.addString("PHY_A");
  uint32_t FwdS = B.addString("phy");
  uint32_t UnsignedEnumS = B.addString("phy");
  uint32_t UnsignedEnumValS = B.addString("phy_a");

  // Type 1: int
  B.addType({IntS, mkInfo(BTF::BTF_KIND_INT), {4}});
  B.addTail((uint32_t)0);
  // Type 2: struct phy { int id; }
  B.addType({PhyStructS, mkInfo(BTF::BTF_KIND_STRUCT) | 1, {4}});
  B.addTail(BTF::BTFMember({FieldS, 1, 0}));
  // Type 3: enum phy { PHY_A = 0 }
  B.addType({SignedEnumS, mkInfo(BTF::BTF_KIND_ENUM) | 1, {4}});
  B.addTail(BTF::BTFEnum({SignedEnumValS, 0}));
  // Type 4: forward declaration of struct phy
  B.addType({FwdS, mkInfo(BTF::BTF_KIND_FWD), {0}});
  // Type 5: enum phy { phy_a = 1 } with different values, so it survives.
  B.addType({UnsignedEnumS, mkInfo(BTF::BTF_KIND_ENUM) | 1, {4}});
  B.addTail(BTF::BTFEnum({UnsignedEnumValS, 1}));

  ASSERT_SUCCEEDED(BTF::dedup(B));
  EXPECT_EQ(B.typesCount(), 4u);

  EXPECT_EQ(B.findType(1)->getKind(), BTF::BTF_KIND_INT);
  EXPECT_EQ(B.findType(2)->getKind(), BTF::BTF_KIND_STRUCT);
  EXPECT_EQ(B.findString(B.findType(2)->NameOff), "phy");
  EXPECT_EQ(B.findType(3)->getKind(), BTF::BTF_KIND_ENUM);
  EXPECT_EQ(B.findType(4)->getKind(), BTF::BTF_KIND_ENUM);
}
...
TEST(BTFDedupTest, recursiveFwdFamilyDedup) {
  BTFBuilder B;
  uint32_t FooFwdS = B.addString("foo");
  uint32_t BarS = B.addString("bar");
  uint32_t MemberS = B.addString("link");

  // Family A:
  //   struct foo;
  //   struct bar { struct foo *link; };
  //   struct foo { struct bar *link; };
  //
  // Family B is a duplicate, but it reaches the same recursion through fully
  // defined structs instead of the forward declaration above. This models the
  // kernel-scale case where equivalent recursive families remain duplicated if
  // composite dedup gets stuck on PTR -> FWD vs PTR -> STRUCT edges.

  // Type 1: forward declaration of struct foo
  B.addType({FooFwdS, mkInfo(BTF::BTF_KIND_FWD), {0}});
  // Type 2: ptr to FWD foo
  B.addType({0, mkInfo(BTF::BTF_KIND_PTR), {1}});
  // Type 3: struct bar { struct foo *link; }
  B.addType({BarS, mkInfo(BTF::BTF_KIND_STRUCT) | 1, {8}});
  B.addTail(BTF::BTFMember({MemberS, 2, 0}));
  // Type 4: ptr to bar
  B.addType({0, mkInfo(BTF::BTF_KIND_PTR), {3}});
  // Type 5: struct foo { struct bar *link; }
  uint32_t FooS = B.addString("foo");
  uint32_t MemberS2 = B.addString("link");
  B.addType({FooS, mkInfo(BTF::BTF_KIND_STRUCT) | 1, {8}});
  B.addTail(BTF::BTFMember({MemberS2, 4, 0}));

  // Family B duplicates the same recursion, but uses full struct references on
  // both sides instead of the FWD from family A.
  // Type 6: ptr to foo (family B)
  B.addType({0, mkInfo(BTF::BTF_KIND_PTR), {8}});
  // Type 7: duplicate struct bar { struct foo *link; }
  uint32_t BarS2 = B.addString("bar");
  uint32_t MemberS3 = B.addString("link");
  B.addType({BarS2, mkInfo(BTF::BTF_KIND_STRUCT) | 1, {8}});
  B.addTail(BTF::BTFMember({MemberS3, 6, 0}));
  // Type 8: duplicate struct foo { struct bar *link; }
  uint32_t FooS2 = B.addString("foo");
  uint32_t MemberS4 = B.addString("link");
  B.addType({FooS2, mkInfo(BTF::BTF_KIND_STRUCT) | 1, {8}});
  B.addTail(BTF::BTFMember({MemberS4, 9, 0}));
  // Type 9: ptr to duplicate bar
  B.addType({0, mkInfo(BTF::BTF_KIND_PTR), {7}});

  EXPECT_EQ(B.typesCount(), 9u);
  ASSERT_SUCCEEDED(BTF::dedup(B));

  // The FWD should resolve to the single surviving struct foo, the duplicate
  // foo/bar families should collapse, and only the two structs plus their
  // pointer types should remain.
  EXPECT_EQ(B.typesCount(), 4u);

  for (uint32_t I = 1; I <= B.typesCount(); ++I)
    EXPECT_NE(B.findType(I)->getKind(), BTF::BTF_KIND_FWD);
}

TEST(BTFDedupTest, recursiveCgroupFamilyDedupForKfuncPrototypes) {
  BTFBuilder B;
  uint32_t IntS = B.addString("int");
  uint32_t CssCpuS = B.addString("css_rstat_cpu");
  uint32_t CgroupS = B.addString("cgroup");
  uint32_t CssS = B.addString("cgroup_subsys_state");
  uint32_t ValueS = B.addString("value");
  uint32_t CgroupMemberS = B.addString("cgroup");
  uint32_t RstatCpuS = B.addString("rstat_cpu");
  uint32_t ParentS = B.addString("parent");
  uint32_t StartS = B.addString("start");
  uint32_t PercpuS = B.addString("percpu");

  // Family A:
  //   struct css_rstat_cpu { int value; };
  //   struct cgroup { int value; };
  //   struct cgroup_subsys_state {
  //     struct cgroup *cgroup;
  //     struct css_rstat_cpu __attribute__((btf_type_tag("percpu"))) *rstat_cpu;
  //     struct cgroup_subsys_state *parent;
  //   };
  B.addType({IntS, mkInfo(BTF::BTF_KIND_INT), {4}});
  B.addTail((uint32_t)0);
  B.addType({CssCpuS, mkInfo(BTF::BTF_KIND_STRUCT) | 1, {4}});
  B.addTail(BTF::BTFMember({ValueS, 1, 0}));
  B.addType({PercpuS, mkInfo(BTF::BTF_KIND_TYPE_TAG), {2}});
  B.addType({0, mkInfo(BTF::BTF_KIND_PTR), {3}});
  B.addType({0, mkInfo(BTF::BTF_KIND_PTR), {6}});
  B.addType({CgroupS, mkInfo(BTF::BTF_KIND_STRUCT) | 1, {4}});
  B.addTail(BTF::BTFMember({ValueS, 1, 0}));
  B.addType({0, mkInfo(BTF::BTF_KIND_PTR), {8}});
  B.addType({CssS, mkInfo(BTF::BTF_KIND_STRUCT) | 3, {24}});
  B.addTail(BTF::BTFMember({CgroupMemberS, 5, 0}));
  B.addTail(BTF::BTFMember({RstatCpuS, 4, 64}));
  B.addTail(BTF::BTFMember({ParentS, 7, 128}));
  B.addType({0, mkInfo(BTF::BTF_KIND_FUNC_PROTO) | 1, {7}});
  B.addTail(BTF::BTFParam({StartS, 7}));

  // Family B is an equivalent recursive graph with different type IDs.
  uint32_t IntS2 = B.addString("int");
  uint32_t CssCpuS2 = B.addString("css_rstat_cpu");
  uint32_t CgroupS2 = B.addString("cgroup");
  uint32_t CssS2 = B.addString("cgroup_subsys_state");
  uint32_t ValueS2 = B.addString("value");
  uint32_t CgroupMemberS2 = B.addString("cgroup");
  uint32_t RstatCpuS2 = B.addString("rstat_cpu");
  uint32_t ParentS2 = B.addString("parent");
  uint32_t StartS2 = B.addString("start");
  uint32_t PercpuS2 = B.addString("percpu");

  B.addType({IntS2, mkInfo(BTF::BTF_KIND_INT), {4}});
  B.addTail((uint32_t)0);
  B.addType({CssCpuS2, mkInfo(BTF::BTF_KIND_STRUCT) | 1, {4}});
  B.addTail(BTF::BTFMember({ValueS2, 10, 0}));
  B.addType({PercpuS2, mkInfo(BTF::BTF_KIND_TYPE_TAG), {11}});
  B.addType({0, mkInfo(BTF::BTF_KIND_PTR), {12}});
  B.addType({0, mkInfo(BTF::BTF_KIND_PTR), {15}});
  B.addType({CgroupS2, mkInfo(BTF::BTF_KIND_STRUCT) | 1, {4}});
  B.addTail(BTF::BTFMember({ValueS2, 10, 0}));
  B.addType({0, mkInfo(BTF::BTF_KIND_PTR), {17}});
  B.addType({CssS2, mkInfo(BTF::BTF_KIND_STRUCT) | 3, {24}});
  B.addTail(BTF::BTFMember({CgroupMemberS2, 14, 0}));
  B.addTail(BTF::BTFMember({RstatCpuS2, 13, 64}));
  B.addTail(BTF::BTFMember({ParentS2, 16, 128}));
  B.addType({0, mkInfo(BTF::BTF_KIND_FUNC_PROTO) | 1, {16}});
  B.addTail(BTF::BTFParam({StartS2, 16}));

  ASSERT_SUCCEEDED(BTF::dedup(B));

  EXPECT_EQ(countTypesByNameAndKind(B, "cgroup_subsys_state",
                                    BTF::BTF_KIND_STRUCT),
            1u);
  EXPECT_EQ(countTypesByNameAndKind(B, "cgroup", BTF::BTF_KIND_STRUCT), 1u);
  EXPECT_EQ(countTypesByNameAndKind(B, "css_rstat_cpu",
                                    BTF::BTF_KIND_STRUCT),
            1u);

  const BTF::CommonType *Proto =
      findTypeByNameAndKind(B, "", BTF::BTF_KIND_FUNC_PROTO);
  ASSERT_TRUE(Proto);
  EXPECT_EQ(countTypesByNameAndKind(B, "", BTF::BTF_KIND_FUNC_PROTO), 1u);
  auto *Params = reinterpret_cast<const BTF::BTFParam *>(
      reinterpret_cast<const uint8_t *>(Proto) + sizeof(BTF::CommonType));
  EXPECT_EQ(Proto->Type, Params[0].Type);
}
...
TEST(BTFDedupTest, funcProtoDedup) {
  ...
  // Duplicate func_proto. Parameter names are not part of the C function type
  // identity and may differ across compilation units.
  // Type 3: int (dup)
  uint32_t IntS2 = B.addString("int");
  B.addType({IntS2, mkInfo(BTF::BTF_KIND_INT), {4}});
  B.addTail((uint32_t)0);
...
TEST(BTFDedupTest, funcDedupPrefersDefinitionOverExtern) {
  BTFBuilder B;
  uint32_t IntS = B.addString("int");
  uint32_t FnS = B.addString("myfunc");
  ...
  // Header declaration before the definition.
  B.addType({FnS, mkInfo(BTF::BTF_KIND_FUNC) | BTF::FUNC_EXTERN, {2}});
  B.addType({FnS, mkInfo(BTF::BTF_KIND_FUNC) | BTF::FUNC_GLOBAL, {2}});

  EXPECT_EQ(B.typesCount(), 4u);
  ASSERT_SUCCEEDED(BTF::dedup(B));
  EXPECT_EQ(B.typesCount(), 3u);

  const BTF::CommonType *Fn =
      findTypeByNameAndKind(B, "myfunc", BTF::BTF_KIND_FUNC);
  ASSERT_NE(Fn, nullptr);
  EXPECT_EQ(Fn->getVlen(), BTF::FUNC_GLOBAL);
}
